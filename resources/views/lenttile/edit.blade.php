...
<div class="container-xxl flex-grow-1 container-p-y" role="document">
    <div class="modal-content card">
        ...
        <form action="{{ url('lenttile/update/' . $data->id) }}" method="post" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="modal-body" id="myform">
              <div class="row">
                ...
                <select name="code_fournisseur" class="form-control">
                  @foreach($fournisseurs as $fournisseur)
                    <option value="{{$fournisseur->id}}" option selected="{{$fournisseur->id}}">{{$fournisseur->code_fournisseur}}</option>
                  @endforeach
                  </select>
              </div>
              ...
              <div class="col mb-0">
                  ...
                  required
                  name="fabriquant_lentille"
                  value="{{ $data['fabriquant_lentille'] }}"
                  />
              </div>
