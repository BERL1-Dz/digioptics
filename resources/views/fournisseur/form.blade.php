<div class="row">
    <div class="col mb-3">
        <label for="nameWithTitle" class="form-label">Code</label>
        <input type="text" id="nameWithTitle" class="form-control" placeholder="Code" name="code_fournisseur" />
    </div>
    <div class="col mb-3">
        <label for="nameWithTitle" class="form-label">Nom</label>
        <input type="text" id="nameWithTitle" class="form-control" placeholder="Nom" name="nom" />
    </div>
    <div class="col mb-3">
        <label for="nameWithTitle" class="form-label">Fabricant associé</label>
        <input type="text" id="nameWithTitle" class="form-control" placeholder="Fabricant associé"
            name="fabricant_associe" />
    </div>
</div>
<div class="row g-2">
    <div class="col mb-0">
        <label for="emailWithTitle" class="form-label">Email</label>
        <input type="email" id="emailWithTitle" class="form-control" placeholder="xxxx@xxx.xx"
            name="mail_fournisseur" />
    </div>
    <div class="col mb-0">
        <label for="emailWithTitle" class="form-label">Telephone</label>
        <input type="number" id="emailWithTitle" class="form-control" placeholder="213666444333"
            name="numero_fournisseur" />
    </div>

</div>
